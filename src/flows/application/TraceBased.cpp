...
TraceBased::Send (void)
{
  ...
  // cout << "Packet Size: " << iter->PacketSize
  // << " Time to Send: " << iter->TimeToSend << endl;
  while (true)
    {
      for (int i = 0; i < (iter->PacketSize) / MAXMTUSIZE; i++)
		{
    	  ...
    	  packet->SetID(uid);
        // cout << "Time: " << Simulator::Init()->Now () << endl;
      ...
      // cout << "Frame Number: " << GetFrameCounter() << endl;
