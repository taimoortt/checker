...
int get_avg_flowsize() {
  ...
  std::cout << "Average flow size: " << avg_flowsize << endl;
  return (int)avg_flowsize;
}

int InternetFlow::m_typeflow = 11;
int InternetFlow::m_flowsize[] = {1460, 2920, 4380, 7300, 10220, 58400, 105120, 200020, 389820, 1082820, 1733020};
...
InternetFlow::InternetFlow(int seed)
{
  SetApplicationType (Application::APPLICATION_TYPE_IPFLOW);
  m_flowCounter = 0;
  rng = new RandomNumberGenerator(0, 1, seed);
}
...
InternetFlow::SetAvgRate(double rate)
{
  ...
  std::cout << "IPFlow rate: " 
  << rate << " mbps; lambda: " 
  << m_lambda << " ; flow size: " 
  << InternetFlow::m_avg_flowsize 
  << std::endl;
}
...
InternetFlow::GetInterval(void)
{
  ...
  double time = std::ceil(interval * 1000) / 1000.0;
